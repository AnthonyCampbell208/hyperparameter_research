...
k = 2
ci_estimators = ['sl', 'tl', 'xl', 'dml',
                 'sparse_dml', 'kernel_dml', 'CausalForestDML']
                        ...
                        for param_y in grid_parameters(select_regression_hyperparameters(model_y)):
                            ...
                            for param_t in grid_parameters(select_classification_hyperparameters(model_t) if is_discrete else select_regression_hyperparameters(model_t)):
                                ...
                                temp_results, estimated_ite_values = causal_inference_analysis(
                                    model_y, model_t, causal_model, x_scaled, Y, T, true_ATE, true_ATE_stderr, true_ite, is_meta, param_y, param_t)
                                ...
                                if i % 50 == 0:
                                    ...
                                    results_df.to_csv(
                                        f'results/{key}_hyper_params_baselines.csv')
                                print(
